...
void format(int _addr){
    sprintf(addr,"%04x",_addr/2);
    fprintf(f,"%s ",addr);
}
    ...
    for( ; i < mem_size + start_add*2 - 1 ; i++){
        ...
        if(i < start_add*2)
            if(state++ < 7)
                ...
                fprintf(f,". "),state=0,state_n++;
        else
            if(state++ < 7)
                fprintf(f,"%c",memory[i-start_add*2]);
            else
                fprintf(f,"%c ",memory[i-start_add*2]),state=0,state_n++;
    }
    fprintf(f,"\n");
    fclose(f);
}
